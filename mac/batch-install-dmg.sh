...
function f_do_dmg_install() {
	...
	then
		...
		if [ $v_count_dest_same_name_app != 0 ]
		then
			v_isok_installcheck="FALSE"
...
function f_check_config() {
	...
	while read -r line
	do
		if [[ "$line" == "$v_head_flag_false" ]]
		then
			echo "⚠️  修改默认配置[${v_head_flag_false}]为[${v_head_flag_true}],才能执行安装"
			exit 1
		elif [[ "$line" == "$v_head_flag_true" ]]
		then
			v_isok_head="TRUE"
		elif [[ "$line" == "$v_version_flag" ]]
		then
			v_isok_version="TRUE"
		fi
		## 检查都通过则 return 0
		if [[ "TRUE" == "$v_isok_version" ]]&&[[ "TRUE" == "$v_isok_head" ]]
		then
			return 0
		fi
...
function f_interactive_menu() {
	select option in "Exit menu(退出)" "Create configuration(生成配置文件)" "Install according to configuration(根据配置文件安装)"
	do
		case $option in
		*)
			## 第一个是 * 的话就是所有参数都会走这里
...
function f_unattended_menu() {
	...
	## 模式校验
	if [[ "TRUE" == "${v_creat_config}" ]]&&[[ "TRUE" == "${v_install}" ]]
	then
		echo "❌  -c 和 -i 不能同时存在"
		exit 1
	fi
	if [[ "FALSE" == "${v_creat_config}" ]]&&[[ "FALSE" == "${v_install}" ]]
	then
		echo "❌  -c 和 -i 必选一种"
		exit 1
	fi
